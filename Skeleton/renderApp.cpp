...
int main( int argc, char *argv[] )
{
    ...
    if(argc==1){
        ...
        //ant obj
        Group* ant = new Group();
        res = loadOBJMTL("Ant_Mesh.obj", ant);
        ant->init();

        //add objects to the scene
        myScene->addObject(person);
        myScene->addObject(sphere);
        myScene->addObject(ant);
    ...
    while( glfwGetKey(window, GLFW_KEY_ESCAPE ) != GLFW_PRESS && glfwWindowShouldClose(window) == 0 ){// Clear the screen
        ...
        //Normal if rendermode = 1
        if (myScene->getRenderMode() == 1) {
            // Clear the screen
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // Also clear the depth buffer!!!

            // update camera controls with mouse input
            myControls->update();
            //float m = myScene->getRenderMode();//change render mode based on key input
        ...
        if (myScene->getRenderMode() == 2) {//box filter
            ...
            // The depth buffer to account for material depth, credit to Lydia A for advice
